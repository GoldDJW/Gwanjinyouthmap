...
function overlayHtml(group,id){const cards=group.items.map((r,i)=>`<details class="inst-card" ${group.items.length===1?'open':''}><summary>${i+1}. ${esc(r.기관명)} <em>${esc(r.이용구분)}</em></summary><div class="badges">${SERVICES.filter(s=>r[s]).map(badge).join('')}</div><div class="detail"><b>주소</b> ${esc(r.주소)}<br>${r.연락처?`<b>연락처</b> ${esc(r.연락처)}<br>`:''}${r.이용대상?`<b>이용대상</b> ${esc(r.이용대상)}<br>`:''}${r.신청방법?`<b>신청방법</b> ${esc(r.신청방법)}<br>`:''}<a href="https://map.kakao.com/link/to/${encodeURIComponent(r.기관명)},${r.위도},${r.경도}" target="_blank" rel="noopener">카카오맵 길찾기</a>${r.홈페이지?` · <a href="${esc(r.홈페이지)}" target="_blank" rel="noopener">홈페이지</a>`:''}</div></details>`).join('');return `<div class="overlay"><div class="overlay-head"><h3>${esc(group.address)}<br><small>${group.items.length}개 기관 · ${group.services.map(esc).join(' · ')}</small></h3><button class="overlay-close" data-overlay-close="${id}" aria-label="닫기">×</button></div><div class="overlay-body">${cards}</div></div>`;}
...
window.GJ={SERVICES,COLORS,DONGS,CFG,esc,n,truth,normDong,normalizeInstitution,dataset,baseAddress,buildingKey,groupBuildings,serviceColor,badge,loadKakao,addMapControls,createMarkerManager,haversine,rowsCsv,download,loadBoundary,renderBoundary,clearOverlays,pointInGeom,dataMeta};
})();
